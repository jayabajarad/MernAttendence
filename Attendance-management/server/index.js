...
const StansardModel = require('./models/Standard');
const TeacherModel = require('./models/Teacher');
const SubjectModel = require('./models/Subject');
const AttendanceModel = require('./models/Attendance2');
app.use(express.json());
app.use(cors());
mongoose.connect('mongodb+srv://jayabajarad9028:Jayabe1234@ecommerce-cluster.gleq1en.mongodb.net/employees?retryWrites=true&w=majority', {
    useNewUrlParser: true,
});

app.post('/insertTeacher', async (req, res) => {
  const name = req.body.name;
  const teacher = new TeacherModel({ name: name});
  try {
      await teacher.save();
...
app.post('/readTeacher', async (req, res) => {
  try {
...
app.post('/attendance', (req, res) => {
  ...
  //const date = new Date().setHours(0, 0, 0, 0); // Get the current date with time set to midnight
    ...
    console.error('recording attendance date:', currentDate);
    const formattedDate = currentDate.toLocaleDateString();
    console.error('recording attendance date:', formattedDate);
    // const parts = formattedDate.split('/'); // Split the string by "/"
    // const date = new Date(`${parts[2]}-${parts[1]}-${parts[0]}`);
    const date = moment(formattedDate, 'M/D/YYYY').toDate();
    console.error('recording attendance date:', date);
